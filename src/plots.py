...
def plot_energy_zigzag_clean(cmes: list[CME_T], filename: str = "plot.png"):
    ...
    data = group_results([cme_to_array_per_mem(cme) for cme in cmes])
    p = BarPlotter(GROUPS, bars, sections, legend_cols=4, ylabel="Energy (pJ)")
    p.plot(data, filename)


def plot_energy_small(cmes: list[CME_T], filename: str = "plot.png"):
    """`cmes` correspond to `LAYERS_TO_PLOT`"""
    assert len(cmes) == 5, "Are these the CMEs from `LAYERS_TO_PLOT`?"
...
def plot_latency_zigzag_clean(cmes: list[CME_T], filename: str = "plot.png"):
    """`cmes` correspond to `LAYERS_TO_PLOT`"""
    assert len(cmes) == 5, "Are these the CMEs from `LAYERS_TO_PLOT`?"
